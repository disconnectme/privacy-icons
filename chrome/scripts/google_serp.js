var runtimeOrExtension, alreadyInjected, byPopupActived;
var ICONS, TLDs;
...
function load_events() {
  ...
  window.addEventListener("message", function(e) {
    if (e.data == 'jrc') {
      //console.log('Chrome-instant callback done');
      chrome[runtimeOrExtension].sendMessage({
        action: "get_options"
      }, function(response) {
        getTruste(response.turnAllOn);
        activePrivacyIcons(response.turnAllOn);
...
function onRuntimeMessage(request, sender, sendResponse) {
  // console.log("onRuntimeMessage: " + request.action);

  if (request.action == "initialize") {
    ICONS = request.icons;
    TLDs = request.tlds;
    customFilters = request.customFilters;
    byPopupActived = request.byPopup;

    getTruste(request.turnAllOn);
    activePrivacyIcons(request.turnAllOn);
    switchIconsVisibility(request.showOption, request.customFilters);
  } else if (request.action == "truste_response") {
    TrusteCheckCallback(request);
  } else if (request.action == "bleed_response") {
    HeartBleedCheckCallback(request);
  } else if (request.action == "sslsupport_response") {
    SSLRedirectCheckCallback(request);
  } else if (request.action == "change_filter_response") {
    switchIconsVisibility(request.showOption, request.customFilters);
  } else if (request.action == "turn_extension_response") {
    getTruste(request.turnAllOn);
    activePrivacyIcons(request.turnAllOn);
    switchIconsVisibility(request.showOption, request.customFilters);
  }
...
function activePrivacyIcons(active) {
  ...
  if (isActive || byPopupActived) {
    ...
    $('#hdtb_msb').css("background-image", "url('" + chrome.extension.getURL("images/disconnect/D.png") + "')");
...
function formatNameIcon(name, joIcon, extra) {
  ...
  var image = chrome.extension.getURL(bgIcon.image + "20" + joIcon.color + ".png");
  ...
  if (joIcon.color == "white") {
    tag = tag + " faded";
    image = chrome.extension.getURL(bgIcon.image + joIcon.color + ".png");
  }
  ...
  if (name == "HeartbleedVulnerable" && extra.isSSL) {
    image = chrome.extension.getURL("images/load.png");
...
function getTruste(active) {
  ...
  chrome[runtimeOrExtension].sendMessage({
    ...
    //console.log("Truste check request_from_serp Callback");
  });
};
...
function TrusteCheckCallback(response) {
  ...
  var hbElements = new Array;
  var sslElements = new Array;
...
function getSSLRedirect() {
  ...
  chrome[runtimeOrExtension].sendMessage({
    ...
    //console.log("SSLRedirect check request_from_serp Callback");
  });
};
...
function SSLRedirectCheckCallback(response) {
  ...
  for (var i in data) {
    ...
    var imageUrl = chrome.extension.getURL(ICONS.SSLSupport.image + "20" + icon.color + ".png");
    if (icon.color == "white" && icon.show) imageUrl = chrome.extension.getURL(ICONS.SSLSupport.image + icon.color + ".png");
...
function getHeartbleed() {
  ...
  chrome[runtimeOrExtension].sendMessage({
    ...
    //console.log("HeartBleed check request_from_serp Callback");
  });
};

function HeartBleedCheckCallback(response) {
  //console.log("HeartBleedCheckCallback", response.data);
  var data = response.data;
  ...
  for (var i in data) {
    ...
    var imageUrl = chrome.extension.getURL(ICONS.HeartbleedVulnerable.image + "20" + icon.color + ".png");
    if (icon.color == "white" && icon.show) imageUrl = chrome.extension.getURL(ICONS.HeartbleedVulnerable.image + icon.color + ".png");
...
function showSubtitles() {
  ...
  html += "</div>";
  $("body").append(html);
  $('#subtitles h2').css("background-image", "url('" + chrome.extension.getURL("images/48.png") + "')");
...
function formatCustomList(name) {
  ...
  var bgIcon = ICONS[name],
      image = chrome.extension.getURL(bgIcon.image + "white.png"),
...
function loadIconsDescriptions() {
  ...
  $('#custom_list li input').on("click", function(event) {
  ...
  // $(".icon_desc_li").click(function() {
  //   if ($('#icon_desc_div').css('display') == 'none') {
  //     $('#icon_desc_div').slideDown();
  //     $('#options_div').slideUp();
  //     $("#subtitles .options_li span").css("-webkit-transform", "rotate(0)");
  //     $("#subtitles .icon_desc_li span").css("-webkit-transform", "rotate(90deg)");
  //   } else {
  //     $('#icon_desc_div').hide();
  //     $("#subtitles .icon_desc_li span").css("-webkit-transform", "rotate(0)");
  //   }
  // });
};
